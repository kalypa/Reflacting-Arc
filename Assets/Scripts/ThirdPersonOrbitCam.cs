...
{
    [SerializeField]
    private UltBehaviour ult;
    [SerializeField]
    private GameObject ultPos;
    ...
    void Update()
    {
        if (!ult.isAtkCam)
        {
            if (cam.GetComponent<Camera>().fieldOfView < defaultFOV)
            {
                Time.timeScale = 1f;
                effect.brightness = 0;
                effect.styleStrength = 0;
        ...
        {
            Time.timeScale = 0.8f;
            if (effect.brightness > -50)
            {
                effect.brightness -= 1;
            }
            if (effect.styleStrength < 1)
            {
                effect.styleStrength += 0.01f;

            }
            if (cam.GetComponent<Camera>().fieldOfView > 25)
            {
                cam.GetComponent<Camera>().fieldOfView -= 0.15f;
            }
            cam.GetComponent<Camera>().gameObject.transform.position = new Vector3(ultPos.transform.position.x, ultPos.transform.position.y, ultPos.transform.position.z);
            cam.GetComponent<Camera>().gameObject.transform.rotation = Quaternion.Euler(14, ultPos.transform.eulerAngles.y, 0);
        }
    }
